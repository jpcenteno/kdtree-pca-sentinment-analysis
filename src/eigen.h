...
std::pair<double, Vector>
    power_iteration(const Matrix& mat, Criterion crit=all, unsigned num_iter=5000, double eps=1e-10);
...
std::pair<Eigen::VectorXd, Matrix>
    get_first_eigenvalues(const Matrix& mat, unsigned num, Criterion crit=all, unsigned num_iter=5000, double epsilon=1e-10);
